    ...
    class CrowMusicEngine {
        ...
        bindUIEvents() {
            ...
            if (seekBar) {
                seekBar.addEventListener('input', (e) => {
                    ...
                    if (this.currentTrack && this.currentTrack.is_cue) {
                        const startTime = this.currentTrack.cue_start_time || 0;
                        const dur = this.currentTrack.duration_sec || (this.audio.duration ? Math.max(0, this.audio.duration - startTime) : 0);
                        if (dur > 0) {
                            this.audio.currentTime = startTime + (pct * dur);
                        }
        ...
        initAudioEngine() {
            this.audio.addEventListener('timeupdate', () => {
                ...
                const isCue = Boolean(this.currentTrack.is_cue);
                const startTime = isCue ? (this.currentTrack.cue_start_time || 0) : 0;
                const trackDur = isCue ? (this.currentTrack.duration_sec || 0) : (this.audio.duration || 0);

                const rawCur = this.audio.currentTime;

                if (isCue) {
                    if (rawCur < startTime && this.audio.duration) {
                        this.audio.currentTime = startTime;
                        return;
                    }

                    const virtualCur = Math.max(0, rawCur - startTime);
                    const effectiveDur = trackDur > 0 ? trackDur : (this.audio.duration ? Math.max(0, this.audio.duration - startTime) : 0);
                    const pct = effectiveDur > 0 ? Math.min(100, Math.max(0, (virtualCur / effectiveDur) * 100)) : 0;

                    // Auto-advance when reaching the end of the virtual track
                    if (effectiveDur > 0 && virtualCur >= effectiveDur) {
                        this.playNext();
                        return;
                    }
                    ...
                    if (curTime) curTime.textContent = this.formatTime(virtualCur);
                    const totalTime = document.getElementById('cmTotalTime');
                    if (totalTime && effectiveDur > 0) totalTime.textContent = this.formatTime(effectiveDur);

                    // Update Now Playing overlay
                    const npSeekBar = document.getElementById('cmNpSeekBar');
                    if (npSeekBar) npSeekBar.value = pct;
                    const npCurTime = document.getElementById('cmNpCurTime');
                    if (npCurTime) npCurTime.textContent = this.formatTime(virtualCur);
                    const npTotTime = document.getElementById('cmNpTotTime');
                    if (npTotTime && effectiveDur > 0) npTotTime.textContent = this.formatTime(effectiveDur);

                    // Karaoke lyrics synchronization relative to virtual track (0:00)
                    this.syncLyricsWithTime(virtualCur);
                } else {
            ...
            this.audio.addEventListener('loadedmetadata', () => {
                if (this.currentTrack && this.currentTrack.is_cue && this.currentTrack.duration_sec > 0) {
                    const totalTime = document.getElementById('cmTotalTime');
                    if (totalTime) totalTime.textContent = this.formatTime(this.currentTrack.duration_sec);
                    const npTotTime = document.getElementById('cmNpTotTime');
                    if (npTotTime) npTotTime.textContent = this.formatTime(this.currentTrack.duration_sec);
                } else {
                    const totalTime = document.getElementById('cmTotalTime');
                ...
                try {
                    ...
                    navigator.mediaSession.setActionHandler('seekto', (details) => {
                        if (details.seekTime != null) {
                            if (this.currentTrack && this.currentTrack.is_cue) {
                                const startTime = this.currentTrack.cue_start_time || 0;
                                this.audio.currentTime = startTime + details.seekTime;
                            } else if (this.audio.duration) {
        ...
        playTrack(track, queue = [], index = 0) {
            ...
            const isCue = Boolean(track.is_cue);
            const parentFileId = isCue && track.parent_file_id ? track.parent_file_id : (track.file_id || track.id);
            const targetSrc = `/api/stream/${parentFileId}`;
            const startTime = isCue && track.cue_start_time ? track.cue_start_time : 0;

            const curSrc = this.audio.src || '';
            const isSameStream = curSrc.includes(`/api/stream/${parentFileId}`);

            if (isSameStream && this.audio.duration) {
                this.audio.currentTime = startTime;
            ...
            // Set initial duration display if known
            const displayDur = isCue && track.duration_sec ? track.duration_sec : 0;
            if (displayDur > 0) {
                const totalTime = document.getElementById('cmTotalTime');
                if (totalTime) totalTime.textContent = this.formatTime(displayDur);
                const npTotTime = document.getElementById('cmNpTotTime');
                if (npTotTime) npTotTime.textContent = this.formatTime(displayDur);
            }
        ...
        playPrev() {
            ...
            const isCue = Boolean(this.currentTrack?.is_cue);
            const startTime = isCue ? (this.currentTrack.cue_start_time || 0) : 0;
            const virtualCur = Math.max(0, this.audio.currentTime - startTime);
