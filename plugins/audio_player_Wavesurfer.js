window.CrowAPI.registerPlugin('WavesurferAudioPlayer', {
    init: function(api) {
    ...
    buildPlaylistFromUI: function() {
        ...
        // Если есть локальный список объектов из БД
        ...
        // Если список пустой — формируем из открытой таблицы на экране
        if (items.length === 0) {
            ...
            rows.forEach(row => {
                ...
                if (cb && nameTd) {
                    const fid = parseInt(cb.getAttribute('data-id'));
                    const fname = nameTd.textContent.trim();
                    const ext = fname.split('.').pop().toLowerCase();
                    if (audioExts.includes(ext)) {
                        items.push({ id: fid, name: fname });
                    }
                }
            });
        }
    ...
    startPlayback: async function(id, name) {
        ...
        // Если трек не найден в глобале, добавляем его вручную
