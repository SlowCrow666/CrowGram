...
async def get_cached_chunk_data(msg_id: int, chat_target, file_id: Optional[int] = None):
    ...
    async with stream_semaphore:
        ...
        if file_id and file_id in stream_progress:
            ...
            stream_progress[file_id]["speed_mbps"] = round((len(data) / (1024 * 1024)) / elapsed, 2)
...
# --- ЭНДПОИНТ ИНФОРМАЦИИ О КЭШИРОВАНИИ / ПОТОКЕ ---
...
# --- ЛОКАЛЬНАЯ ФАЙЛОВАЯ СИСТЕМА ДЛЯ CROWCOMMANDER ---
...
def _scan_dir_safe(target_path: Path):
    ...
    try:
        ...
        print(f"[WARN] Ошибка сканирования директории {target_path}: {e}")
    return items
...
async def api_app_auth_recover():
    ...
    if tg_manager.is_authorized() and info["raw_password"]:
        try:
            ...
            print(f"[WARN] Ошибка отправки пароля в Telegram: {e}")
...
async def download_zip(ids: str = Query(...), name: Optional[str] = Query("archive")):
    ...
    headers = {
        "Content-Disposition": f"attachment; filename=\"{ascii_fallback}.zip\"; filename*=UTF-8''{encoded_utf8_name}"
    }
